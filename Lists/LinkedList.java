...
public class LinkedList<E> implements List<E> {
    ...
    // pre : 0 <= index < size()
    // post: returns the node at a specific index. Uses the fact that the list is doubly-linked to start from the front or the back, whichever is closer.
    private ListNode<E> nodeAt(int index) {
        ListNode<E> current;
        // if index falls in the first half of the list, start from the front
        if (index < size / 2) {
            current = front;
        ...
        else { // if (index > size / 2) start from the back
            current = back;
    ...
    private class LinkedIterator implements Iterator<E> {
        ...
        // pre : next() has been called without a call on remove (i.e., at most one call per call on next)
